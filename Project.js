...
}}function applyNarrativeFlowEnhancements(){var storyCard=document.getElementById("storyCard");
var choices;
var recap;
var sceneParagraphs;
var i;
var extraWrapper;
if(!storyCard||currentScene<=0){return;
}choices=storyCard.querySelector("#choices");
sceneParagraphs=Array.prototype.slice.call(storyCard.querySelectorAll(":scope > p"));
if(sceneParagraphs.length<2){recap=document.createElement("p");
...
if(choices){storyCard.insertBefore(recap,choices);
}else{storyCard.appendChild(recap);
}}sceneParagraphs=Array.prototype.slice.call(storyCard.querySelectorAll(":scope > p"));
if(sceneParagraphs.length>4){extraWrapper=document.createElement("details");
extraWrapper.className="extended-scene-notes";
extraWrapper.innerHTML="<summary>Extended scene notes</summary>";
for(i=4;
i<sceneParagraphs.length;
i++){extraWrapper.appendChild(sceneParagraphs[i]);
}if(choices){storyCard.insertBefore(extraWrapper,choices);
}else{storyCard.appendChild(extraWrapper);
...
}}else if(currentScene===54){storyCard.innerHTML="<h2>War Council</h2>"+"<p>The rescue campaign enters its strategic phase. Messengers arrive with sketches of cliff routes, tidal windows, and guard rotations along Lanka's perimeter.</p>"+"<p>Hanuman asks for patience: one more focused mission could convert scattered clues into war-grade intelligence.</p>"+"<div id='choices'>"+"<button onclick='makeChoice(47)'>Return to Hanuman's camp</button>"+"<button onclick='makeDecision(3)'>Lead the next story mission</button>"+"</div>";
}else if(currentScene===55){addArtifact("Ocean Wind Compass");
storyCard.innerHTML="<h2>The Journey Across the Ocean</h2>"+"<p>The rescue council stretches late into the night. Engineers propose rafts, archers propose covering fire, and navigators argue over storm timing. Jambavan's conclusion is clear: Hanuman's leap must be the first decisive move, but it must be framed inside a coordinated campaign, not a lone gamble.</p>"+"<p>You receive the Ocean Wind Compass, a command tool tuned to relay corridor changes between scouts and launch teams.</p>"+"<p><strong>This is currently the final available story scene.</strong></p>"+"<div id='choices'>"+"<button onclick='restart()'>Restart Journey</button>"+"<button onclick='makeChoice(47)'>Return to Camp Hub</button>"+"</div>";
...
}else if(currentScene===54&&decision===3){currentScene=55;
}finishSceneDecision(previousScene);
}document.addEventListener("DOMContentLoaded",function(){console.log("Update 36");
