...
function renderTimeline(containerId, showHighlight) {
    ...
    for (i = 0; i < timelineEdges.length; i++) {
        ...
        svg += "<text class='edge-label' x='" + labelX + "' y='" + labelY + "'>" + escapeHtml(edge.label) + "</text>";
    }
    ...
    for (i = 0; i < timelineLevels.length; i++) {
        ...
        for (j = 0; j < levelNodes.length; j++) {
            ...
            svg += "<text class='node-title' x='" + (coords[nodeId].x + 10) + "' y='" + (coords[nodeId].y + 37) + "'>" +
                escapeHtml(timelineNodeTitles[nodeId] || "Scene") + "</text>";
        }
    }
...
function renderTimelines(showHighlight) {
    var replayWrapper = document.getElementById("timelineReplayWrapper");

    renderTimeline("timelineFlowchart", false);
    ...
    if (showHighlight) {
        ...
        renderTimeline("timelineFlowchartHighlight", true);
    } else {
...
function showScene() {
    ...
    makeReceipt();
    setUndoButton();
    renderTimelines(isTerminalScene(currentScene));
}
